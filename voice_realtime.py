...
async def process_audio_buffer(buffer: np.ndarray):
    ...
    start_total = time.time()   # ⏱️ البداية الكلية
    start_stt = time.time()
    ...
    buffer_text = ""
    try:
        for chunk in run_agent_stream(user_text=text, session=session):
            buffer_text += chunk
        end_llm = time.time()
        ...
        # ⏱️ طباعة القياسات بالمللي ثانية
        ...
        print(f"\n⏱️ Latency → STT: {stt_ms} ms | LLM: {llm_ms} ms | TTS: {tts_ms} ms | TOTAL: {total_ms} ms\n")
        print("✅ Done speaking.\n")
